...
void batch_training(int input, int output, int data, int each_layer, int middle_layer, double** x_input, double* y_output, double*** w, double epsilon)
{
	...
	double sum_r2[10][10][10] = { 0 };
	double E[10] = { 0 };//誤差
	double sum_E = 0;//誤差の合計
	double grad[10][10][10] = { 0 };//勾配
	do {
		sum_E = 0;//誤差の合計の初期化
		for (i = 0;i < 10;i++) {
			E[i] = 0;
		...
		for (l = 0;l < data;l++) {
			...
			if (middle_layer == 1) {//中間層が1のとき
				for (i = 0;i < output;i++) {
					for (j = 0;j < each_layer + 1;j++) {
						if (j == each_layer)
							sum_u[l][i][1] += w[j][i][1];//バイアス
						else
							sum_u[l][i][1] += I[l][j][1] * w[j][i][1];
					}
					I[l][i][2] = sigmoid(sum_u[l][i][1]);
					printf("Y = %lf ", I[l][i][2]);
				}
				for (i = 0;i < output;i++) {
					E[l] += pow(I[l][i][2] - y_output[l], 2) / 2.0;
				}
			}
			//中間層が1より大きい時
			else {
				...
				for (i = 0;i < output;i++) {
					for (j = 0;j < each_layer + 1;j++) {
						if (j == each_layer)
							sum_u[l][i][k] += w[j][i][k];//バイアス
						else
							sum_u[l][i][k] += I[l][j][k] * w[j][i][k];
					}
					I[l][i][k+1] = sigmoid(sum_u[l][i][k]);
					printf("Y = %lf ", I[l][i][k+1]);
				}
				for (i = 0;i < output;i++) {
					E[l] += pow(I[l][i][k+1] - y_output[l], 2) / 2.0;
				}
			}
		}
		for (i = 0;i < data;i++) {
		...
		for (i = 0;i < each_layer;i++) {
			for (j = 0;j < each_layer;j++) {
				for (l = 0;l < data;l++) {
					grad[j][i][k] += sum_r2[l][i][k] * I[l][j][k];
				}
			}
		}
		//中間層が1つの場合
	...
	} while (sum_E > 0.0001 && cnt < 5000000);
	printf("学習の終了\n");
	printf("count = %d  error = %f \n", cnt, sum_E);
	if (cnt == 50000) {
		printf("count = %d\n", cnt);
